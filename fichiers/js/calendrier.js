document.addEventListener('DOMContentLoaded', () => {
    ...
    const dayActionModal = document.getElementById('dayActionModal'); // Nouveau
    ...
    // NOUVEAU : Affiche la modale d'action pour un jour donné
    function showDayActionModal(dateStr, events) {
        ...
        // Formate le titre avec la date cliquée
        const date = new Date(dateStr + 'T00:00:00'); // Assure l'interprétation en date locale
        titleEl.textContent = date.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' });

        // Vide la liste précédente
        listEl.innerHTML = '';

        // Remplit la liste avec les événements du jour
    ...
    const closeDayActionModal = () => { // Nouveau
    ...
    document.querySelector('#dayActionModal .close-day-action-btn').addEventListener('click', closeDayActionModal); // Nouveau
    
    window.addEventListener('click', e => {
        if (e.target == eventModal) closeEventModal();
        if (e.target == eventDetailsModal) closeDetailsModal();
        if (e.target == dayActionModal) closeDayActionModal(); // Nouveau
        if (e.target == customAlertModal && customAlertModal.style.display === 'flex') {
            // ne rien faire
        }
    });

    // Boutons de la modale de détails
